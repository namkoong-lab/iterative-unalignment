...
def compute_population_ess_from_log_weights(log_importance_weights: torch.Tensor) -> float | None:
    ...
    if torch.all(log_importance_weights == log_importance_weights[0]):
        # Identical weights are the maximum-ESS case: every sample contributes
        # equally.  This is especially important on the initial Q=P step, where
        # all log(P/Q) weights are exactly zero.
        return 1.0
...
def compute_worst_case_log_ratio_per_pos(
    ...
    # log P(v) - log Q(v) over the vocabulary, then max over v. In-place subtract
    # keeps peak memory at ~2x[B,K,V] rather than 3x.
