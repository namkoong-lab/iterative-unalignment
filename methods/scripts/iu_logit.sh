#!/usr/bin/env bash
# GPT-2 IU logit-steering Token Presence via runner.py.
set -euo pipefail
...
MODEL_ID="gpt2"
PROPOSAL_TYPE="IU_LOGIT"
EVENT_TYPE="token"
EVENT_CONFIG_JSON='{"surrogate":"sum"}'
OUTPUT_PATH="${OUTPUT_PATH:-${METHODS_DIR}/../runs/iu_logit_token}"
...
SEED=120
LOG_EVERY=25
IU_LOGIT_STEERING_INIT_SCALE=0.01
...
CMD=(
  ...
  --seed "$SEED"
  --log_every "$LOG_EVERY"
  --iu_logit_steering_init_scale "$IU_LOGIT_STEERING_INIT_SCALE"
  --no-use_lora
)
