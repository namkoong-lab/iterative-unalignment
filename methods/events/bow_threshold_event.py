...
class BowThresholdEvent(RareEvent):
    ...
    def from_config(cls, tokenizer, config: Mapping[str, Any]) -> "BowThresholdEvent":
        ...
        # Same key as TokenEvent for train/metrics plumbing. There is no built-in
        # GT table for bow_threshold yet — supply a measured or placeholder rate.
    ...
    def diagnostic_counts(
        ...
        # No "cheat-position" notion for BoW threshold events.
        return {}

    def compute_score(self, generated_tokens: torch.Tensor) -> torch.Tensor:
        """Literal climbing score for CE: the BoW classifier logit
        z = wᵀc + b on the DISCRETE realized unigram counts c (the exact
        quantity the event thresholds, since the event is sigmoid(z) >= thr,
        i.e. z >= logit_threshold). Higher z = closer to / past the threshold.
        CE is gradient-free so it ranks elites by this hard score directly,
        instead of the differentiable expected-count surrogate IU optimises.
        Returns [B]."""
        if generated_tokens.ndim != 2:
            raise ValueError(
                f"generated_tokens must have shape [B, K], got {tuple(generated_tokens.shape)}"
    ...
    def compute_surrogate_loss(
        ...
        seq_k = a.shape[-1]
        loss_values = -(torch.logsumexp(a, dim=-1) - math.log(seq_k)) - b
