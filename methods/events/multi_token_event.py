...
class MultiTokenEvent(RareEvent):
    ...
    # Note: deliberately NO `token_id` attribute, so train.py's single-token
    # per-position diagnostic (getattr(rare_event, "token_id", None)) skips cleanly.
    token: str
    ...
    def compute_surrogate_loss(
        ...
        if self.surrogate_mode == "sequence":
            ...
            # The paper normalizes by the full generation length L; positions
            # after sequence completion contribute zero through the active mask.
            ...
            # -mean_k log( sum_i p_k^i ); logsumexp over the target log-probs is
            # the numerically stable log of the per-position union mass (targets
            # are mutually exclusive within a single sampled position).
