...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional alpha regularization moment list to iterate over. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If omitted, each token is trained with gt_prob=0 (metrics that need a reference are skipped/meaningless). "
...
def main() -> None:
    ...
    alpha_reg_moment_values = (
        _parse_float_values(args.alpha_reg_moment, "--alpha_reg_moment")
        if args.alpha_reg_moment is not None
        else [None]
    )
    ess_target_values = (
    ...
    if args.token_pairs_file is not None and args.token_gt_file is not None:
        # token_gt_file is ignored; fail fast so shell mistakes are obvious.
    ...
    for run_event_config in run_event_configs:
        ...
        for alpha_reg_moment in alpha_reg_moment_values:
            for ess_target in ess_target_values:
                for init_lambda in init_lambda_values:
                    override_args: list[str] = []
                    if alpha_reg_moment is not None:
                        override_args.extend(["--alpha_reg_moment", str(alpha_reg_moment)])
                    ...
                    print(
                        ...
                        f"{run_desc}, alpha_reg_moment={alpha_reg_moment}, "
